...
| **MAP-117** | Crear formulario simple de alta de SpaceElement | ✅ `space-element-form.ts` + `space-element-list.ts` + ruta `spaces/sectors/:sectorId/elements`. Sin drag&drop/editor visual (eso es HU-4.01). Reutiliza tokens de `ui-kit` y el patrón feature → model/data/ui.                                                   |
| **MAP-118** | Integrar formulario de SpaceElement con API     | ✅ `spaces.api` llama al `api-client` generado (`createSpaceElement`/`updateSpaceElement`/`listSpaceElementsBySector`) con error/loading gestionados en el store; el HttpClient solo está permitido en `data/`.                                                    |
